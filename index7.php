...
<div v-if="selectedPlayer" class="bg-white p-4 rounded-lg shadow mb-4">
    ...
    <p>{{ fieldNames['military'] || '军力水平' }}: {{ playerStats.military }}</p >
    <p>{{ fieldNames['culture'] || '文化影响力' }}: {{ playerStats.culture }}</p >
    <p>{{ fieldNames['science'] || '科技进展' }}: {{ playerStats.science }}</p >
    <p>{{ fieldNames['infrastructure'] || '基础建设' }}: {{ playerStats.infrastructure }}</p >
    <p>{{ fieldNames['health'] || '公共卫生' }}: {{ playerStats.health }}</p >
    <p>{{ fieldNames['education'] || '教育水平' }}: {{ playerStats.education }}</p >
...
<div v-if="selectedCity" class="city-details bg-white p-4 rounded-lg shadow mt-4">
    ...
    <p v-if="selectedCity.military && selectedCity.military !== 'N/A'">{{ fieldNames['military'] || '军力水平' }}: {{ selectedCity.military }} (增长率: {{ selectedCity.military_growth || 0 }}%)</p >
    <p v-if="selectedCity.culture && selectedCity.culture !== 'N/A'">{{ fieldNames['culture'] || '文化影响力' }}: {{ selectedCity.culture }} (增长率: {{ selectedCity.culture_growth || 0 }}%)</p >
    <p v-if="selectedCity.science && selectedCity.science !== 'N/A'">{{ fieldNames['science'] || '科技进展' }}: {{ selectedCity.science }} (增长率: {{ selectedCity.science_growth || 0 }}%)</p >
    <p v-if="selectedCity.infrastructure && selectedCity.infrastructure !== 'N/A'">{{ fieldNames['infrastructure'] || '基础建设' }}: {{ selectedCity.infrastructure }} (增长率: {{ selectedCity.infrastructure_growth || 0 }}%)</p >
    <p v-if="selectedCity.health && selectedCity.health !== 'N/A'">{{ fieldNames['health'] || '公共卫生' }}: {{ selectedCity.health }} (增长率: {{ selectedCity.health_growth || 0 }}%)</p >
    <p v-if="selectedCity.education && selectedCity.education !== 'N/A'">{{ fieldNames['education'] || '教育水平' }}: {{ selectedCity.education }} (增长率: {{ selectedCity.education_growth || 0 }}%)</p >
    <p v-if="selectedCity.stability && selectedCity.stability !== 'N/A'">{{ fieldNames['stability'] || '社会稳定' }}: {{ selectedCity.stability }} (增长率: {{ selectedCity.stability_growth || 0 }}%)</p >
...
try {
    ...
    fetch('view_orders.php?game_id=<?php echo $game_id; ?>&type=public', { cache: 'no-store' })
        ...
        .then(data => {
            console.log('Raw data from view_orders.php:', data); // 调试原始数据
            ...
            console.log('Parsed publicOrders:', this.publicOrders); // 调试解析后数据
        })
        .catch(error => {
