...
function evaluateFormula($formula, $city, $allowed_fields = [
    ...
    'infrastructure', 'health', 'education', 'stability'
]) {
    // 将字段名替换为实际值
...
// 在 Handle city/tile creation 部分
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['end_round'])) {
    try {
        ...
        foreach ($cities as $city) {
            ...
            // Calculate increments for economy and resources
            $economy_increment = evaluateFormula($formulas['economy'] ?? '0', $city);
            $resources_increment = evaluateFormula($formulas['resources'] ?? '0', $city);

            // Calculate new values (cumulative for economy and resources)
            $new_values = [
                'population' => ($city['population'] ?? 0) + (($city['population'] ?? 0) * ($growth_rates['growth_rate'] / 100)),
                'resources' => ($city['resources'] ?? 0) + $resources_increment,
                'economy' => ($city['economy'] ?? 0) + $economy_increment,
                'military' => ($city['military'] ?? 0) + (($city['military'] ?? 0) * ($growth_rates['military_growth'] / 100)),
                'culture' => ($city['culture'] ?? 0) + (($city['culture'] ?? 0) * ($growth_rates['culture_growth'] / 100)),
                'science' => ($city['science'] ?? 0) + (($city['science'] ?? 0) * ($growth_rates['science_growth'] / 100)),
                'infrastructure' => ($city['infrastructure'] ?? 0) + (($city['infrastructure'] ?? 0) * ($growth_rates['infrastructure_growth'] / 100)),
                'health' => ($city['health'] ?? 0) + (($city['health'] ?? 0) * ($growth_rates['health_growth'] / 100)),
                'education' => ($city['education'] ?? 0) + (($city['education'] ?? 0) * ($growth_rates['education_growth'] / 100)),
                'stability' => ($city['stability'] ?? 0) + (($city['stability'] ?? 0) * ($growth_rates['stability_growth'] / 100)),
            ];
            ...
            $stmt->execute([
                ...
                $growth_rates['military_growth'],
                $growth_rates['culture_growth'],
                $growth_rates['science_growth'],
                $growth_rates['infrastructure_growth'],
                $growth_rates['health_growth'],
                $growth_rates['education_growth'],
                $growth_rates['stability_growth'],
                $city['id']
            ]);
        }
...
<div class="bg-white p-4 rounded-lg shadow mb-8">
    <?php
    // Load all cities and their player tags
    $stmt = $pdo->prepare("
...
<form method="POST" class="mb-8 bg-white p-4 rounded-lg shadow">
    ...
    <label class="block text-sm font-medium mb-1">军力水平 (原: 军事+military字段)</label>
    <input type="text" name="field_names[military]" value="<?php echo htmlspecialchars($field_names['military'] ?? '军力水平'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1">文化影响力 (原: 文化+culture字段)</label>
    <input type="text" name="field_names[culture]" value="<?php echo htmlspecialchars($field_names['culture'] ?? '文化影响力'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1">科技进展 (原: 科技+science字段)</label>
    <input type="text" name="field_names[science]" value="<?php echo htmlspecialchars($field_names['science'] ?? '科技进展'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1">基础建设 (原: 基础设施+infrastructure字段)</label>
    <input type="text" name="field_names[infrastructure]" value="<?php echo htmlspecialchars($field_names['infrastructure'] ?? '基础建设'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1">公共卫生 (原: 健康+health字段)</label>
    <input type="text" name="field_names[health]" value="<?php echo htmlspecialchars($field_names['health'] ?? '公共卫生'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1">教育水平 (原: 教育+education字段)</label>
    <input type="text" name="field_names[education]" value="<?php echo htmlspecialchars($field_names['education'] ?? '教育水平'); ?>" class="w-full p-2 mb-2 border rounded">
...
<form method="POST" class="mb-8 bg-white p-4 rounded-lg shadow">
    ...
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['military'] ?? '军力水平'); ?>增长率公式</label>
    <input type="text" name="formulas[military_growth]" value="<?php echo htmlspecialchars($formulas['military_growth'] ?? 'economy * 0.005 + stability * 0.01'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['culture'] ?? '文化影响力'); ?>增长率公式</label>
    <input type="text" name="formulas[culture_growth]" value="<?php echo htmlspecialchars($formulas['culture_growth'] ?? 'education * 0.015 + stability * 0.005'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['science'] ?? '科技进展'); ?>增长率公式</label>
    <input type="text" name="formulas[science_growth]" value="<?php echo htmlspecialchars($formulas['science_growth'] ?? 'education * 0.02 + infrastructure * 0.01'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['infrastructure'] ?? '基础建设'); ?>增长率公式</label>
    <input type="text" name="formulas[infrastructure_growth]" value="<?php echo htmlspecialchars($formulas['infrastructure_growth'] ?? 'economy * 0.01 + resources * 0.005'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['health'] ?? '公共卫生'); ?>增长率公式</label>
    <input type="text" name="formulas[health_growth]" value="<?php echo htmlspecialchars($formulas['health_growth'] ?? 'infrastructure * 0.01 + stability * 0.01'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['education'] ?? '教育水平'); ?>增长率公式</label>
    <input type="text" name="formulas[education_growth]" value="<?php echo htmlspecialchars($formulas['education_growth'] ?? 'science * 0.015 + culture * 0.01'); ?>" class="w-full p-2 mb-2 border rounded">
    <label class="block text-sm font-medium mb-1"><?php echo htmlspecialchars($field_names['stability'] ?? '社会稳定'); ?>增长率公式</label>
    <input type="text" name="formulas[stability_growth]" value="<?php echo htmlspecialchars($formulas['stability_growth'] ?? 'health * 0.01 + education * 0.01'); ?>" class="w-full p-2 mb-2 border rounded">
