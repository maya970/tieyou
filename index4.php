...
// Handle city name visibility update
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_city_name'])) {
    $city_id = $_POST['city_id'] ?? null;
    $show_name = isset($_POST['show_name']) ? 1 : 0;

    if (!$city_id || !is_numeric($city_id)) {
        header("Location: index.php?game_id=$game_id&error=无效的城市ID。");
        exit;
    }

    try {
        // Verify city belongs to the game
        $stmt = $pdo->prepare("SELECT id FROM cities WHERE id = ? AND game_id = ?");
        $stmt->execute([$city_id, $game_id]);
        if (!$stmt->fetch()) {
            header("Location: index.php?game_id=$game_id&error=无效的城市。");
            exit;
        }

        // Update show_name
        $stmt = $pdo->prepare("UPDATE cities SET show_name = ? WHERE id = ? AND game_id = ?");
        $stmt->execute([$show_name, $city_id, $game_id]);

        header("Location: index.php?game_id=$game_id&success=城市名称显示已更新。");
        exit;
    } catch (Exception $e) {
        error_log("更新城市名称显示失败: " . $e->getMessage());
        header("Location: index.php?game_id=$game_id&error=更新失败: " . htmlspecialchars($e->getMessage()));
        exit;
    }
}
?>
...
<body class="bg-gray-100 min-h-screen">
    <div id="app" class="container mx-auto p-4">
        ...
        <div v-if="selectedCity" class="city-details bg-white p-4 rounded-lg shadow mt-4">
            ...
            <p>人口: {{ selectedCity.population || 'N/A' }} (增长: {{ selectedCity.growth_rate || 0 }}%)</p>
            <p>资源: {{ selectedCity.resources || 'N/A' }}</p>
            <p>经济: {{ selectedCity.economy || 'N/A' }} (增长: {{ selectedCity.economy_growth || 0 }}%)</p>
            <p>军事: {{ selectedCity.military || 'N/A' }} (增长: {{ selectedCity.military_growth || 0 }}%)</p>
            <p>文化: {{ selectedCity.culture || 'N/A' }} (增长: {{ selectedCity.culture_growth || 0 }}%)</p>
            <p>科技: {{ selectedCity.science || 'N/A' }} (增长: {{ selectedCity.science_growth || 0 }}%)</p>
            <p>基础设施: {{ selectedCity.infrastructure || 'N/A' }} (增长: {{ selectedCity.infrastructure_growth || 0 }}%)</p>
            <p>健康: {{ selectedCity.health || 'N/A' }} (增长: {{ selectedCity.health_growth || 0 }}%)</p>
            <p>教育: {{ selectedCity.education || 'N/A' }} (增长: {{ selectedCity.education_growth || 0 }}%)</p>
            <p>稳定性: {{ selectedCity.stability || 'N/A' }} (增长: {{ selectedCity.stability_growth || 0 }}%)</p>
            ...
            <!-- Toggle City Name Visibility -->
            <?php if ($_SESSION['role'] === 'game_admin' && $game['creator_id'] == $_SESSION['user_id'] || $_SESSION['role'] === 'super_admin'): ?>
            <form method="POST" class="mt-2">
                <input type="hidden" name="toggle_city_name" value="1">
                <input type="hidden" name="city_id" :value="selectedCity.id">
                <label class="flex items-center">
                    <input type="checkbox" name="show_name" :checked="selectedCity.show_name" class="mr-2">
                    在地图上显示城市名称
                </label>
                <button type="submit" class="mt-2 bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">更新名称显示</button>
            </form>
            <?php endif; ?>
            ...
            const app = Vue.createApp({
                data() {
                    return {
                        ...
                        startX: 0,
                        startY: 0,
                        showCityNames: <?php echo $game['show_city_names'] ? 1 : 0; ?>,
                ...
                methods: {
                    ...
                    startDrag(event) {
                        ...
                        this.startY = clientY - this.startY;
                    },
                    drag(event) {
