...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['autorpg_game_id'])) {
        ...
        if (!$stmt->fetch()) {
            $x = rand(0, 9); // Adjusted for 10x10 grid
            $y = rand(0, 9);
            $stmt = $pdo->prepare("INSERT INTO autorpg_players (game_id, user_id, x, y, layer, moves_left) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$game_id, $user_id, $x, $y, 1, 3]); // Start at layer 1
        }
