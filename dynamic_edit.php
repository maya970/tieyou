...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_city'])) {
    ...
    try {
        ...
        $optional_fields = [
            'population', 'resources', 'growth_rate', 'economy', 'military', 'military_growth',
            'culture', 'culture_growth', 'science', 'science_growth', 'infrastructure',
            'infrastructure_growth', 'health', 'health_growth', 'education', 'education_growth',
            'stability', 'stability_growth', 'food_consumption', 'money_consumption'
        ];
        $params = [];
        foreach ($optional_fields as $field) {
            $params[$field] = !empty(trim($_POST[$field] ?? '')) && is_numeric($_POST[$field]) ? $_POST[$field] : null;
        }
        ...
        $stmt->execute([
            ...
            $params['population'],
            $params['resources'],
            $_POST['type'] === 'city' ? $params['growth_rate'] : null,
            $_POST['city_display_type'] ?? ($_POST['type'] === 'city' ? 'circle' : 'text'),
            trim($_POST['city_display_value'] ?? ''),
            $_POST['type'] === 'city' ? $params['economy'] : null,
            $_POST['type'] === 'city' ? $params['military'] : null,
            $_POST['type'] === 'city' ? $params['military_growth'] : null,
            $_POST['type'] === 'city' ? $params['culture'] : null,
            $_POST['type'] === 'city' ? $params['culture_growth'] : null,
            $_POST['type'] === 'city' ? $params['science'] : null,
            $_POST['type'] === 'city' ? $params['science_growth'] : null,
            $_POST['type'] === 'city' ? $params['infrastructure'] : null,
            $_POST['type'] === 'city' ? $params['infrastructure_growth'] : null,
            $_POST['type'] === 'city' ? $params['health'] : null,
            $_POST['type'] === 'city' ? $params['health_growth'] : null,
            $_POST['type'] === 'city' ? $params['education'] : null,
            $_POST['type'] === 'city' ? $params['education_growth'] : null,
            $_POST['type'] === 'city' ? $params['stability'] : null,
            $_POST['type'] === 'city' ? $params['stability_growth'] : null,
            isset($_POST['show_name']) ? 1 : 0,
            $_POST['color'] ?: null,
            $params['food_consumption'],
            $params['money_consumption'],
            $city_id,
            $game_id
        ]);
            ...
            <div class="modal-content">
                ...
                <form @submit.prevent="submitCityUpdate" class="space-y-2">
                    ...
                    <div class="city-fields space-y-2">
                        ...
                        <input type="text" v-model="selectedCity.population" name="population" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                        <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['resources'] ?? '物资储备'); ?></label>
                        <input type="text" v-model="selectedCity.resources" name="resources" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                        <label class="block text-sm font-medium">耗粮</label>
                        <input type="text" v-model="selectedCity.food_consumption" name="food_consumption" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                        <label class="block text-sm font-medium">耗钱</label>
                        <input type="text" v-model="selectedCity.money_consumption" name="money_consumption" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                        <template v-if="selectedCity.type === 'city'">
                            ...
                            <input type="text" v-model="selectedCity.growth_rate" name="growth_rate" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['economy'] ?? '财富指数'); ?></label>
                            <input type="text" v-model="selectedCity.economy" name="economy" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['military'] ?? '军力水平'); ?></label>
                            <input type="text" v-model="selectedCity.military" name="military" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['military'] ?? '军力水平'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.military_growth" name="military_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['culture'] ?? '文化影响力'); ?></label>
                            <input type="text" v-model="selectedCity.culture" name="culture" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['culture'] ?? '文化影响力'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.culture_growth" name="culture_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['science'] ?? '科技进展'); ?></label>
                            <input type="text" v-model="selectedCity.science" name="science" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['science'] ?? '科技进展'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.science_growth" name="science_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['infrastructure'] ?? '基础建设'); ?></label>
                            <input type="text" v-model="selectedCity.infrastructure" name="infrastructure" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['infrastructure'] ?? '基础建设'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.infrastructure_growth" name="infrastructure_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['health'] ?? '公共卫生'); ?></label>
                            <input type="text" v-model="selectedCity.health" name="health" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['health'] ?? '公共卫生'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.health_growth" name="health_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['education'] ?? '教育水平'); ?></label>
                            <input type="text" v-model="selectedCity.education" name="education" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['education'] ?? '教育水平'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.education_growth" name="education_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['stability'] ?? '社会稳定'); ?></label>
                            <input type="text" v-model="selectedCity.stability" name="stability" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                            <label class="block text-sm font-medium"><?php echo htmlspecialchars($field_names['stability'] ?? '社会稳定'); ?>增长率 (%)</label>
                            <input type="text" v-model="selectedCity.stability_growth" name="stability_growth" pattern="^[0-9]*\.?[0-9]*$|^$" class="w-full p-2 border rounded">
                        </template>
                    </div>
                    <label class="block">
            ...
            methods: {
                selectCity(city) {
                    ...
                    this.selectedCity = {
                        ...
                        military_growth: city.military_growth ?? '',
                        culture: city.culture ?? '',
                        culture_growth: city.culture_growth ?? '',
                        science: city.science ?? '',
                        science_growth: city.science_growth ?? '',
                        infrastructure: city.infrastructure ?? '',
                        infrastructure_growth: city.infrastructure_growth ?? '',
                        health: city.health ?? '',
                        health_growth: city.health_growth ?? '',
                        education: city.education ?? '',
                        education_growth: city.education_growth ?? '',
                        stability: city.stability ?? '',
                        stability_growth: city.stability_growth ?? '',
                        food_consumption: city.food_consumption ?? '',
                        money_consumption: city.money_consumption ?? ''
                    };
                },
                toggleCityFields() {
                    if (this.selectedCity.type !== 'city') {
                        ['growth_rate', 'economy', 'military', 'military_growth',
                         ...
                         'health', 'health_growth', 'education', 'education_growth', 'stability', 'stability_growth']
                            .forEach(field => this.selectedCity[field] = '');
                    }
                },
                submitCityUpdate() {
                    ...
                    for (const field of numericFields) {
                        const value = this.selectedCity[field];
                        if (value !== '' && isNaN(value)) {
                            alert(`字段 ${field} 必须是数字或空值。`);
                            return;
                        }
                    }
